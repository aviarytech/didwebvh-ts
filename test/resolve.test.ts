...
describe('Resolver URL derivation', () => {
  test('Uses https for localhost DID host', () => {
    ...
    expect(buildDidLogUrl(did)).toBe('https://localhost:8000/test/path/did.jsonl');
  });
  ...
  test('Uses https for non-localhost DID host', () => {
    ...
    expect(buildDidLogUrl(did)).toBe('https://example.com:8080/custom/path/did.jsonl');
  });
  ...
  test('Uses .well-known did.jsonl when DID has no path', () => {
    ...
    expect(buildDidLogUrl(did)).toBe('https://example.com/.well-known/did.jsonl');
  });
